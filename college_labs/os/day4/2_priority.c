...
int main() {
    int at[N], bt[N], priority[N], completed=0, current_time=0;
    int gantt[MAX], gantt_idx=0;
    int tat[N], wt[N];
    printf("Enter arrival time, burst time, and priority for 5 processes:\n");
    printf("lower number = highest priority\n");
    for (int i=0; i<N; i++) {
        printf("P%d Arrival Time : ", i+1); scanf("%d", &at[i]);
        printf("P%d Burst Time : ", i+1); scanf("%d", &bt[i]);
        printf("P%d Priority : ", i+1); scanf("%d", &priority[i]);
    }
    ...
    while (completed < N) {
        ...
        // Find process with highest priority and arrived by current_time
        for (int i=0; i<N; i++) {
    ...
    for(int i=0; i<N; i++){
        printf("P%d      |   %2d    |  %2d   |    %2d    |     %2d     |    %2d\n",
               i+1, at[i], bt[i], priority[i], tat[i], wt[i]);
        total_tat += tat[i];
        total_wt += wt[i];
    }
